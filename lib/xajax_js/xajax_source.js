...
function ###AJAXNAME###_Xajax()
{
	...
	{
		if (###AJAXNAME###_xajaxDebug) this.DebugMessage("Initializing Request Object..");
		var req = null;
	...
	this.$ = function(sId)
	{
		...
		if (###AJAXNAME###_xajaxDebug && !returnObj && sId != this.workId) {
	...
	this.objectToXML = function(obj)
	{
		...
		for (i in obj)
		{
			...
			{
				if (###AJAXNAME###_xajaxDebug) this.DebugMessage(e.name+": "+e.message);
			}
		}
		sXml += "</xjxobj>";
	...
	{
		var i,r,postData;
		if (document.body && ###AJAXNAME###_xajaxWaitCursor)
			document.body.style.cursor = 'wait';
		if (###AJAXNAME###_xajaxStatusMessages == true) window.status = 'Sending Request...';
		clearTimeout(loadingTimeout);
		loadingTimeout = setTimeout("###AJAXNAME###.loadingFunction();",400);
		if (###AJAXNAME###_xajaxDebug) this.DebugMessage("Starting xajax...");
		if (sRequestType == null) {
		   var xajaxRequestType = ###AJAXNAME###_xajaxDefinedPost;
		}
		else {
			var xajaxRequestType = sRequestType;
		}
		var uri = ###AJAXNAME###_xajaxRequestUri;
		var value;
		switch(xajaxRequestType)
		{
			case ###AJAXNAME###_xajaxDefinedGet:{
				var uriGet = uri.indexOf("?")==-1?"?###AJAXNAME###_xajax="+encodeURIComponent(sFunction):"&###AJAXNAME###_xajax="+encodeURIComponent(sFunction);
				if (aArgs) {
					for (i = 0; i<aArgs.length; i++)
					{
						...
						if (typeof(value)=="object")
							value = this.objectToXML(value);
						uriGet += "&###AJAXNAME###_xajaxargs[]="+encodeURIComponent(value);
					}
				}
				uriGet += "&###AJAXNAME###_xajaxr=" + new Date().getTime();
				uri += uriGet;
				postData = null;
				} break;
			case ###AJAXNAME###_xajaxDefinedPost:{
				postData = "###AJAXNAME###_xajax="+encodeURIComponent(sFunction);
				postData += "&###AJAXNAME###_xajaxr="+new Date().getTime();
				if (aArgs) {
					for (i = 0; i <aArgs.length; i++)
					{
						value = aArgs[i];
						if (typeof(value)=="object")
							value = this.objectToXML(value);
						postData = postData+"&###AJAXNAME###_xajaxargs[]="+encodeURIComponent(value);
					}
				}
				} break;
			default:
		...
		if (!r) return false;
		r.open(xajaxRequestType==###AJAXNAME###_xajaxDefinedGet?"GET":"POST", uri, true);
		if (xajaxRequestType == ###AJAXNAME###_xajaxDefinedPost)
		{
			try
			{
		...
		r.onreadystatechange = function()
		{
			...
			if (r.status==200)
			{
				if (###AJAXNAME###_xajaxDebug) ###AJAXNAME###.DebugMessage("Received:\n" + r.responseText);
				if (r.responseXML && r.responseXML.documentElement)
					###AJAXNAME###.processResponse(r.responseXML);
				else {
					...
					if (###AJAXNAME###_xajaxStatusMessages == true) window.status = 'Invalid XML response error';				
				}
			}
			else {
				if (###AJAXNAME###.responseErrorsForAlert.containsValue(r.status)) {
		...
		if (###AJAXNAME###_xajaxDebug) this.DebugMessage("Calling "+sFunction +" uri="+uri+" (post:"+ postData +")");
		r.send(postData);
		if (###AJAXNAME###_xajaxStatusMessages == true) window.status = 'Waiting for data...';
		delete r;
		return true;
	}
	...
	this.processResponse = function(xml)
	{
		...
		if (###AJAXNAME###_xajaxStatusMessages == true) window.status = 'Processing...';
		var tmpXajax = null;
		...
		delete i;
		document.body.style.cursor = 'default';
		if (###AJAXNAME###_xajaxStatusMessages == true) window.status = 'Done';
	}
}

var ###AJAXNAME### = new  ###AJAXNAME###_Xajax();
###AJAXNAME###_xajaxLoaded = true;
