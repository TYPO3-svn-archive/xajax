 ...
 *
 *  (c) 2007 Ricardo Mieres (ricardo.mieres@502.cl)
 *  All rights reserved
 *
...
/**
 * The tx_xajaxmp_xajax class generates the xajax javascript for your page including the 
...
require_once(t3lib_extMgm::extPath('xajax') .'lib/class.tx_xajax_response.php');
require_once(t3lib_extMgm::extPath('xajax') .'lib/xajaxCompress.php');
...
class tx_xajax extends tx_xajax_xajax{
	function tx_xajax($sRequestURI="",$sWrapperPrefix="xajax_",$sEncoding=XAJAX_DEFAULT_CHAR_ENCODING,$bDebug=false){
		 $this->init('xajax',$sRequestURI,$sWrapperPrefix,$sEncoding,$bDebug);
	}
} 

class tx_xajax_xajax {
	...
	var $iPos;
	/**
	 * @var string Name of the xajax object "obligatory"
	 * 
	 */
	var $sAjaxName;
	...
	/**
	 * Init. You can set some extra tx_xajax options right away or use (Contructor t3lib_div::mekeIntance)
	...
	function init($sAjaxName='set_me_a_name',$sRequestURI="",$sWrapperPrefix="xajax_",$sEncoding=XAJAX_DEFAULT_CHAR_ENCODING,$bDebug=false)
	{
		
		$this->sAjaxName=$sAjaxName;
	...
	{
		if (!empty($_GET[$this->sAjaxName."_xajax"]))
			return XAJAX_GET;
		
		if (!empty($_POST[$this->sAjaxName."_xajax"]))
			return XAJAX_POST;
	...
	{	
		...
		{
			$sFunctionName = $_POST[$this->sAjaxName."_xajax"];
			
			if (!empty($_POST[$this->sAjaxName."_xajaxargs"])) 
				$aArgs = $_POST[$this->sAjaxName."_xajaxargs"];
		}
		else
		{	
			...
			$sFunctionName = $_GET[$this->sAjaxName."_xajax"];
			
			if (!empty($_GET[$this->sAjaxName."_xajaxargs"])) 
				$aArgs = $_GET[$this->sAjaxName."_xajaxargs"];
		}
		...
		if ($bFoundFunction) {
			...
			{
				$bFoundFunction = false;
				$objResponse = new tx_xajaxmp_response();
		...
		if ($this->bErrorHandler && !empty( $GLOBALS['xajaxErrorHandlerText'] )) {
			$sErrorResponse = new tx_xajaxmp_response();
	...
	function getJavascriptConfig()
	{
		...
		$html .= "var ".$this->sAjaxName."_xajaxRequestUri=\"".$this->sRequestURI."\";\n";
		$html .= "var ".$this->sAjaxName."_xajaxDebug=".($this->bDebug?"true":"false").";\n";
		$html .= "var ".$this->sAjaxName."_xajaxStatusMessages=".($this->bStatusMessages?"true":"false").";\n";
		$html .= "var ".$this->sAjaxName."_xajaxWaitCursor=".($this->bWaitCursor?"true":"false").";\n";
		$html .= "var ".$this->sAjaxName."_xajaxDefinedGet=".XAJAX_GET.";\n";
		$html .= "var ".$this->sAjaxName."_xajaxDefinedPost=".XAJAX_POST.";\n";
		$html .= "var ".$this->sAjaxName."_xajaxLoaded=false;\n";
	...
	{
		return $this->getTempJavascript();
	}
	
	/**
	 * Returns a string containing a Javascript include of the js file
	 * along with a check to see if the file loaded after six seconds
	 * (typically called internally by xajax from get/printJavascript).
	 * 
	 * 
	 * @return string
	 */
	function getTempJavascript(){
		$sJsFile ="lib/xajax_js/xajax_source.js";
		
		if (!file_exists($sJsFile)) {
			$jsContent=file_get_contents (t3lib_extMgm::siteRelPath('xajax').$sJsFile);
			$jsContent=str_replace('###AJAXNAME###',$this->sAjaxName,$jsContent);
			$tempJsName='typo3temp/xajax_'.md5($this->sAjaxName).'.js';
			$fp = fopen($tempJsName, 'w');
			fwrite($fp, $jsContent);
			fclose($fp);
  			$html = "\t<script type=\"text/javascript\" src=\"" . $tempJsName . "\"></script>\n";	
			$html .= "\t<script type=\"text/javascript\">\n";
			$html .= "window.setTimeout(function () { if (!".$this->sAjaxName."_xajaxLoaded) { alert('Error: the xajax Javascript file could not be included. Perhaps the URL is incorrect?\\nURL: {$sJsURI}{$sJsFile}'); } }, 6000);\n";
			$html .= "\t</script>\n";
			return $html;	
		}
	}
	...
	/**
	 * Returns the current URL based upon the SERVER vars (for typo3).
	 * 
	 * @access private
	 * @return string
	 */
	...
	function _detectURI(){
		$no_cache=t3lib_div::GPvar('no_cache');
		if($no_cache==1){
			$uri=$_SERVER['SCRIPT_NAME'].'?'.$_SERVER['QUERY_STRING'];
		}else{
			$uri=$_SERVER['SCRIPT_NAME'].'?'.$_SERVER['QUERY_STRING'].'&no_cache=1';
		}	
	...
	{
		$js = "function ".$this->sWrapperPrefix."$sFunction(){return ".$this->sAjaxName.".call(\"$sFunction\", arguments, ".$sRequestType.");}\n";		
		return $js;
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/xajax/lib/class.tx_xajax_xajax.php'])	{
  include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/xajax/lib/class.tx_xajax_xajax.php']);
}
